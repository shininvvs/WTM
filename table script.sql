...
CREATE TABLE tb_user (
    ...
    email VARCHAR(255) UNIQUE,             -- 이메일 (고유값, 중복 방지)
    ...
    -- ✅ 성별 추가
    user_gender ENUM('male', 'female') NOT NULL DEFAULT 'male', -- 성별 (기본값: male)
    ...
    profile_img VARCHAR(255) NULL,     -- 소셜 로그인 프로필 이미지
...
-- ✅ 방 테이블 생성
CREATE TABLE ROOMS (
  room_id INT AUTO_INCREMENT PRIMARY KEY,   -- AUTO_INCREMENT로 자동 증가 설정
  room_name VARCHAR(255) NOT NULL,
  room_pwd VARCHAR(20),
  current_users int default 1,
  current_state VARCHAR(1) default 'N' check (current_state in ('Y', 'N')) -- 현재 방 상태 (n:준비상태/y:게임진행중)
);
